...
def sim_algorithm_time_power():
    ...
    for pev in EV_list:
        ...
        print("\n================================================================")
        print("ID {}    S:{}   D:{}  Time:{}".format(pev.id, pev.source, pev.destination, pev.t_start))
        update_envir_time_power(CS_list, simple_graph, sim_time)
        final_path = []
        weight = []
        final_path.append(pev.curr_location)
        ...
        while pev.next_location != pev.destination or pev.charged != 1:
            ...
            paths_info = routing_method_time_power(CS_list, pev, simple_graph)
            paths_info.sort(key=lambda element: element[0])
            ...
            pev.next_location = path[1]
            print(pev.charged, len(paths_info), pev.traveltime,
                  simple_graph.dict_edges[pev.curr_location][pev.next_location], path)
            final_path.append(pev.next_location)
            weight.append(simple_graph.dict_edges[pev.curr_location][pev.next_location]["weight"])
            update_ev(pev, simple_graph)

        if pev.traveltime >100:
            print(pev.traveltime)
            input()
        print(final_path)
        print(weight)
        sum_weight = sum(weight)
...
if __name__ == "__main__":
    ...
    #     test.append(a)
    # # t1 = np.random.normal(60, 6, 1000)
    # # t2 = np.random.normal(120, 12, 1000)
    #
    # t1 = np.random.random_integers(0, 99, 1000)
    # t2 = np.random.random_integers(0, 99, 1000)
    #
    #
    # # count, bins, ignored = plt.hist(test, 20, density=True)
    # count, bins, ignored = plt.hist(t1, 100, density=True, label="road velocity(60km)")
    # count, bins, ignored = plt.hist(t2, 100, density=True, label="road velocity(120km)")
    # plt.legend()
    # plt.show()